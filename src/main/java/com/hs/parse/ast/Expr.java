...
public class Expr extends ASTNode {
    ...
    public Expr(ASTNodeType type, Token lexeme) {
        super();
        this.type = type;
    ...
    private static ASTNode E_(int k, PeekTokenIterator it) throws ParseException {
        ...
        if(table.get(k).indexOf(value) != -1) {
            var expr = new Expr(ASTNodeType.BINARY_EXPR, it.nextMatch(value));
    ...
    private static ASTNode U(PeekTokenIterator it) throws ParseException {
        ...
        if(value.equals("(")) {
            ...
            var t = it.peek();
            it.nextMatch(value);
            Expr unaryExpr = new Expr(ASTNodeType.UNARY_EXPR, t);
    ...
    private static ASTNode combine(PeekTokenIterator it, ExprHOF aFunc, ExprHOF bFunc) throws ParseException {
        ...
        Expr expr = new Expr(ASTNodeType.BINARY_EXPR, b.lexeme);
        expr.addChild(a);
